...
export class ViewRecipe {
  ...
  viewRelatedRecipePage(id:string,name:string,cuisine:string){
    this.recipe$ = this.api.viewRecipeAPI(id)
    this.getAllRelatedRecipes(name,cuisine)
  ...
  generatePDF(recipe:any){
    let pdf = new jsPDF()
    let titleRow = [['Name','Cuisine','Ingredients','Instruction','Calories','Servings']]
